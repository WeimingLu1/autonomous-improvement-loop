...
_MAINTENANCE_CANDIDATES: list[dict] = [
    {
        ...
        "execution_plan": ["python3 -m pytest tests/ --collect-only -q 2>/dev/null | grep '<Function' | wc -l", 'identify untested scripts/*.py', 'add pytest test for most critical untested function'], "verification": ['python3 -m pytest tests/test_maintenance_mode.py tests/test_task_planner.py -q'], "non_goals": ['不覆盖所有边界条件', '不重写已有测试'], "scope": ["tests/", "scripts/"],
    },
    {
        ...
        "execution_plan": ['ls tests/test_*.py', 'identify missing integration tests between modules', 'add integration test in tests/ directory'], "verification": ['python3 -m pytest tests/ -q --ignore=tests/test_llm 2>&1 | tail -3'], "non_goals": ['不覆盖所有边界条件', '不重写已有测试'], "scope": ["tests/", "scripts/"],
    },
    {
        ...
        "execution_plan": ["pip list --outdated 2>/dev/null || pip3 list --outdated 2>/dev/null || echo 'check ok'", 'check requirements.txt and pyproject.toml', 'flag if security advisories exist for current versions'], "verification": ['python3 -c "import scripts.init; print(\'import ok\')"'], "non_goals": ['不强制升级到最新版本', '不破坏向后兼容'], "scope": ["requirements.txt", "pyproject.toml", "scripts/"],
    },
    {
        ...
        "execution_plan": ["grep -rn 'os.system\\|subprocess.call.*shell=True\\|eval(\\|pickle.load' scripts/ 2>/dev/null || echo 'none found'", 'check for hardcoded secrets in git history', 'report security findings if any found'], "verification": ["python3 -m py_compile scripts/init.py && echo 'syntax ok'"], "non_goals": ['不进行完整的渗透测试', '不修复发现的问题（交给后续 bug 任务）'], "scope": ["scripts/", "tests/"],
    },
    {
        ...
        "execution_plan": ["grep -n 'def [a-z][a-z]*\\|[a-z] = ' scripts/*.py | head -30", 'identify unclear names (single letters, vague names)', 'rename if unambiguous improvement'], "verification": ["python3 -m py_compile scripts/*.py && echo 'syntax ok'"], "non_goals": ['不进行大规模重构', '不改变函数签名'], "scope": ["scripts/"],
    },
    {
        ...
        "execution_plan": ["grep -n 'def.*:\\s*$' scripts/*.py | head -20", 'identify functions without try/except blocks', 'add basic exception handling where critical'], "verification": ['python3 -c "from scripts.init import *; print(\'ok\')"'], "non_goals": ['不覆盖所有边界条件', '不改变函数返回值'], "scope": ["scripts/", "tests/"],
    },
    {
        "title": "完善日志语句，提升可调试性",
        "task_type": "maintenance",
        ...
        "execution_plan": ["grep -rn 'print(' scripts/*.py | grep -v debug | head -20", 'identify missing logging statements', 'add logging where appropriate'], "verification": ["python3 -m py_compile scripts/*.py && echo 'ok'"], "non_goals": ['不添加过多日志', '不改变业务逻辑'], "scope": ["scripts/"],
    },
    {
        ...
        "execution_plan": ['python3 -c \'import timeit; print(timeit.timeit("sum(range(1000))", number=1000))\'', 'identify slow startup functions', 'optimize if clear bottleneck found'], "verification": ['python3 -c "from scripts import init; print(\'import ok\')"'], "non_goals": ['不进行复杂的性能测试', '不引入新依赖'], "scope": ["scripts/", "tests/"],
    },
    {
        ...
        "execution_plan": ['git log --oneline -3', 'check if README reflects recent changes', 'update README.md if major changes not reflected'], "verification": ["test -f README.md && echo 'readme exists'"], "non_goals": ['不重写整个文档', '不添加无关内容'], "scope": ["README.md", "docs/"],
    },
    {
        ...
        "execution_plan": ['git status --short', 'identify unused files or dead code', 'clean up if safe to remove'], "verification": ['git status --short'], "non_goals": ['不删除用户代码', '不强制删除有引用依赖的文件'], "scope": ["scripts/", "tests/"],
    },
    {
        ...
        "execution_plan": ['python3 -m pytest tests/ -x -q 2>/dev/null | head -20', 'identify failing tests or edge cases', 'fix root cause if clear'], "verification": ['python3 -m pytest tests/test_maintenance_mode.py -q'], "non_goals": ['不重写测试框架', '不改变功能行为'], "scope": ["scripts/", "tests/"],
    },
    {
        "title": "提升配置灵活性，减少硬编码",
        "task_type": "maintenance",
        ...
        "execution_plan": ["grep -rn 'TODO\\|FIXME\\|HACK\\|XXX' scripts/*.py | head -20", 'identify hardcoded values and TODOs', 'add to config if unambiguous'], "verification": ['python3 -c "from scripts.config import *; print(\'ok\')"'], "non_goals": ['不破坏现有配置', '不引入复杂的配置系统'], "scope": ["scripts/", ".ail/config.md"],
    },
    {
        ...
        "execution_plan": ["grep -rn 'def .*\\n.*return' scripts/*.py | head -10", 'identify duplicate patterns', 'extract common functions if clear benefit'], "verification": ["python3 -m py_compile scripts/*.py && echo 'ok'"], "non_goals": ['不进行大规模重构', '不改变函数签名'], "scope": ["scripts/"],
    },
    {
        ...
        "execution_plan": ['python3 -m pytest tests/ --collect-only -q 2>/dev/null | tail -3', 'verify all tests are runnable', 'fix if any test is broken'], "verification": ['python3 -m pytest tests/test_maintenance_mode.py -q'], "non_goals": ['不修复所有测试', '不添加复杂验证流程'], "scope": ["scripts/", "tests/", "README.md"],
    },
    {
        ...
        "execution_plan": ['ls scripts/*.py', 'identify gaps in tooling', 'add script if clear need found'], "verification": ['ls scripts/*.py | wc -l'], "non_goals": ['不重写已有工具', '不添加复杂脚本'], "scope": ["scripts/"],
    },
]
